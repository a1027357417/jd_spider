...
class jd_spider(Spider):
    ...
    def parse_price(self, response):
        ...
        temp1 = response.text.split('jQuery([')
    ...
    def parse_detail(self, response):
        ...
        #print(response.body)
        temp = response.text.split('commentVersion:')
    ...
    def parse(self, response):  # 解析搜索页
        ...
        for good in goods:
            item1 = goodsItem()
            item1['ID'] = good.xpath('./div/@data-sku').extract()[0]
            item1["name"]= good.xpath('./div/div[@class="p-name"]/a/em/text()').extract()[0]

            item1['shop_name'] = good.xpath('./div/div[@class="p-shop"]/@data-shop_name').extract()[0]
